# Backend Code Standards - C#/.NET Web API

## Purpose

These standards define coding, structure, and database conventions for **C#/.NET Web API projects**, focusing on predictable folder layout, EF Core configuration, DTOs, controllers, and Swagger integration. They are optimized for automated generation tools while enforcing clean, maintainable code.
...
### Package Version Management (Directory.Package.props)

- Use `Directory.Packages.props` in the solution root for centralized package version management.
- Add `<ManagePackageVersionsCentrally>true</ManagePackageVersionsCentrally>` to each `.csproj` file's PropertyGroup.
